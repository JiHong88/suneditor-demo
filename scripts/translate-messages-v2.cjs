/**
 * Translates messages/en.json → messages/{locale}.json (incremental, hash-based)
 * Uses Google Cloud Translation API (official).
 *
 * Requires: GOOGLE_APPLICATION_CREDENTIALS env var pointing to service account JSON.
 *
 * Usage:
 *   node scripts/translate-messages-v2.cjs              # translate all site locales (incremental)
 *   node scripts/translate-messages-v2.cjs ko           # translate specific locale only
 *   node scripts/translate-messages-v2.cjs --force      # force re-translate everything
 */
...
const SRC = path.join(MSG_DIR, "en.json");
const HASH_FILE = path.join(MSG_DIR, ".messages-hashes.json");
...
// ── Collect all leaf string values from nested JSON ───────
function collectStrings(obj, result, segments) {
	if (typeof obj === "string") {
		result.push({ segments: [...segments], text: obj });
		return;
	}
...
// ── Get/Set value by path segments ────────────────────────
...
async function translateLocale(locale, data, strings, localeHashes, force) {
	...
	const OUT = path.join(MSG_DIR, `${locale}.json`);
	...
	for (let i = 0; i < strings.length; i++) {
		...
		const { protected: p, tokens } = protectPlaceholders(s.text);
		toTranslate.push({ index: i, protectedText: p, tokens });
	}

	console.log(`\n[${"=".repeat(50)}]`);
	console.log(`[messages] Translating → "${locale}" (${targetLang})`);
	console.log(
	...
	for (let i = 0; i < toTranslate.length; i += BATCH_SIZE) {
		...
		try {
			...
			for (let j = 0; j < batch.length; j++) {
				const item = batch[j];
				if (batchResults[j]) {
					results[item.index] = restorePlaceholders(batchResults[j], item.tokens);
				} else {
	...
	// Build output JSON preserving structure of en.json
...
async function main() {
	...
	console.log(`[messages] Target locales: ${locales.join(", ")}${force ? " (force)" : ""}`);

	const data = JSON.parse(fs.readFileSync(SRC, "utf-8"));
	const strings = [];
	collectStrings(data, strings, []);
	console.log(`Found ${strings.length} translatable strings in en.json.`);
