...
const HIDDEN_ROUTES = ["/playground", "/docs-api"];
...
export default function Footer({ version = "" }: FooterProps) {
	...
	if (HIDDEN_ROUTES.some((route) => pathname.includes(route))) return null;
	...
	return (
		<footer role='contentinfo' className='pb-20 border-t border-slate-200/60 dark:border-slate-700/50 bg-slate-50/60 dark:bg-slate-900/30'>
			<div className='max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-3 gap-6 text-sm text-slate-600 dark:text-slate-300'>
				...
				<nav>
					<h4 className='font-semibold mb-2 text-slate-900 dark:text-slate-100'>{t("resources")}</h4>
					<ul className='space-y-1'>
						<li className='my-4'>
							<Link
								href='/feature-demo'
				...
				<div>
					<h4 className='font-semibold mb-2 text-slate-900 dark:text-slate-100'>
						<svg viewBox='0 0 24 24' width='16' height='16' aria-hidden='true' className='opacity-80 inline-block align-text-bottom me-1'>
							<path
								fill='currentColor'
					...
					<p className='my-4 mb-2'>
						<Link href='https://github.com/JiHong88/suneditor?tab=MIT-1-ov-file' target='_blank'>{t("license")}</Link>
					</p>
					<p className='my-4 flex items-center gap-2'>
						<Link href='https://github.com/JiHong88/suneditor/graphs/contributors' target='_blank'>
