...
import { BookOpen, Search, ExternalLink, ArrowRight, Zap, Code2, Settings, Puzzle, Globe, Shield, Download, FileText, Video, Headphones, Clock, Users } from "lucide-react";
...
export default function DocsPage() {
	...
	const filteredSections = Object.entries(documentationSections).filter(
		([key, section]) =>
			section.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
			section.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
			section.items.some((item) => item.title.toLowerCase().includes(searchQuery.toLowerCase()) || item.description.toLowerCase().includes(searchQuery.toLowerCase())),
	);
		...
		<div className='min-h-screen bg-gradient-to-b from-background to-muted/20'>
			...
			<section className='container mx-auto px-6 py-12'>
				<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className='text-center max-w-4xl mx-auto'>
					...
					<h1 className='text-4xl font-bold tracking-tight md:text-5xl'>SunEditor Documentation</h1>
							...
							<Card key={link.href} className='group hover:shadow-md transition-shadow cursor-pointer'>
								<CardContent className='p-4'>
									<div className='flex items-center gap-3'>
										<div className='p-2 bg-primary/10 rounded-lg text-primary'>{link.icon}</div>
								...
								<div className='grid md:grid-cols-2 gap-4'>
									{section.items.map((item) => (
										<div key={item.href} className='group'>
											<Link href={item.href} className='block p-4 rounded-lg border border-transparent hover:border-border hover:bg-muted/50 transition-all'>
					...
					{filteredSections.length === 0 && (
						<Card>
							<CardContent className='p-8 text-center'>
								...
								<p className='text-muted-foreground'>Try searching for something else or browse the sections above.</p>
							</CardContent>
						</Card>
					)}
				</motion.section>
					...
					<div className='grid md:grid-cols-2 gap-6'>
						<Card>
							<CardHeader>
								...
								<CardDescription>Get help from the community and contribute to discussions</CardDescription>
							</CardHeader>
							<CardContent className='space-y-3'>
								...
								<Button variant='outline' className='w-full justify-start' asChild>
									<Link href='https://github.com/JiHong88/SunEditor/issues' target='_blank'>
										Report Issues
										<ExternalLink className='ml-auto h-4 w-4' />
									</Link>
								</Button>
								<Button variant='outline' className='w-full justify-start' asChild>
									<Link href='https://stackoverflow.com/questions/tagged/suneditor' target='_blank'>
										Stack Overflow
										<ExternalLink className='ml-auto h-4 w-4' />
									</Link>
								</Button>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>Keep up with the latest updates and releases</CardDescription>
							</CardHeader>
							<CardContent className='space-y-3'>
								<Button variant='outline' className='w-full justify-start' asChild>
								...
								<Button variant='outline' className='w-full justify-start' asChild>
									<Link href='https://github.com/JiHong88/SunEditor/releases' target='_blank'>
										GitHub Releases
										<ExternalLink className='ml-auto h-4 w-4' />
									</Link>
								</Button>
